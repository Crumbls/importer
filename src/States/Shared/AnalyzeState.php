...
class AnalyzeState extends AbstractState
{
    ...
    protected function analyzePostTypes($storage): array
    {
        ...
        foreach ($posts as $post) {
            ...
            if (!isset($postTypeGroups[$postType])) {
            ...
            $postTypeGroups[$postType]['total_count']++;
            
            if (!isset($postTypeGroups[$postType]['statuses'][$status])) {
                $postTypeGroups[$postType]['statuses'][$status] = 0;
            }
            $postTypeGroups[$postType]['statuses'][$status]++;

            // Keep a few sample posts for reference
            if (count($postTypeGroups[$postType]['sample_posts']) < 3) {
                $postTypeGroups[$postType]['sample_posts'][] = [
